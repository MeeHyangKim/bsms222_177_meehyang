...
```{r}
ca <- us_contagious_diseases %>%   filter(state == "California" & weeks_reporting>=10) %>%  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>% 
  mutate(state = reorder(state, rate))
ca %>% ggplot() + geom_line(aes(year, rate, col=disease), size=1) + scale_y_continuous(trans="sqrt", breaks=c(5,25,125)) + ggtitle("Cases per 10,000 in California") +   xlab("") + ylab("")
```
...
```{r}
d %>% ggplot (aes(team,expression)) + geom_boxplot(aes(col=team))
d %>% mutate(is_autosome=ifelse(seqnames%in%c("chrM","chrY","ChrX"), "N","Y"),"NA")
```
