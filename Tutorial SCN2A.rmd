...
```{r}
length(which(c$Freq>"1"))
c_d <- as.data.frame(table(d$c.DNA))
length(which(c_d$Freq > "1"))
Pos_d <- as.data.frame(table(d$Pos_hg19))
length(which(Pos_d$Freq > "1"))
```
...
```{r}
cl <- as.data.frame(table(d$Classification))
mutate(cl,total=Freq/293 * 100)
```
Then, you might be intrigued to whether females and males have different occurrence in each disorder. Let's check it.
```{r}
df <- filter(d,PatientSex == "F")
df_cl <- as.data.frame(table(Female_Class=df$Classification))
dm <- filter(d,PatientSex =="M")
dm_cl <- as.data.frame(table(Male_Class=dm$Classification))
mutate(df_cl)
mutate (dm_cl)
```
...
```{r}
as.data.frame(table(Phenotype=d$Classification,Mutation_consequences=d$Effect)) %>% arrange(Phenotype)
```
...
```{r}
i <- 1:293
p <- gsub('[^0-9]', '', d[i, 'c.DNA'])
p.d<- as.data.frame(table(c.DNApositions=p))
filter(p.d,Freq>1)
```
