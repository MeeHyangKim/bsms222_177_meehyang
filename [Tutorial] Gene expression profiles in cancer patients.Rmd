...
* TP: Primary solid 
* TumorTR: Recurrent solid 
* TumorNT: Solid Tissue Normal
...
```{r}
#> Q1 : the missing values are replaced by 0.
#> Q2 : function facet_wrap() wraps a 1d sequence of panels into 2d. [scales = 'free_x'] makes the scales free in one dimension.
```
...
yes, it is a godo visulaization for the distribution.
...
```{r}
d_luad %>% filter(!is.na(cigarettes_per_day)) %>% ggplot(aes(cigarettes_per_day, fill=tumor_stage)) + geom_histogram()
```
