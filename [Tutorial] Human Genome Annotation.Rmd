...
d %>% filter(type=="transcript") %>%
  count(gene_id)%>% summarise(mean(n),quantile(n,0.25),quantile(n,0.50),quantile(n,0.75),gene_id[which.max(n)])
```
...
```{r}
d %>% filter(gene_type%in%c("pseudogenes","protein-coding", "lncRNA")) %>% group_by(gene_type,gene_id) %>% count() 
```

3. Final task is to compute the number of transcripts per gene per chromosome.
```{r}
d %>% filter(type=="transcript") %>% 
  count(seqnames,gene_id)
```

## 3.2. Gene length in the GENCODE
1. What is the average length of human genes?
```{r}
d %>% filter(type=="gene") %>% summarise(average=mean(width))
```
...
```{r}
b1 <- d %>% filter(seqnames!="chrM") %>% mutate(c_type = case_when(seqnames %in% c("chrX","chrY") ~ "sex", TRUE ~ "autosomal"))
b1 %>% group_by(c_type) %>% summarise (width = quantile(width,c(0,0.25,0.50,0.75,1)), .groups ="keep") %>% mutate(q=c(0,0.25,0.50,0.75,1))
```
...
```{r}
d %>% group_by(gene_type) %>% summarise(width = quantile(width,c(0,0.25,0.50,0.75,1)), .groups="keep") %>% mutate(q=c(0,0.25,0.50,0.75,1))
```
...
```{r}
d %>% group_by(transcript_support_level) %>% filter(type == "transcript") %>% count()
```

2. From the first question, please count the number of transcript for each TSL by gene biotype.
```{r}
d %>% group_by(gene_type,transcript_support_level) %>% filter(type == "transcript") %>% count()
```

3. From the first question, please count the number of transcript for each TSL by source.
```{r}
d %>% group_by(source,transcript_support_level) %>% filter(type == "transcript") %>% count()
```
...
```{r}
b2 <- d %>% select(gene_id, gene_name, hgnc_id, gene_type, seqnames, start, end, strand) %>% mutate(isHgnc=case_when(is.na(hgnc_id)~"0", TRUE~"1")) %>% mutate (isCCDS=case_when(is.na(d$ccdsid) ~"0", TRUE ~"1"))
b2
```
2. Please count the number of hgnc by gene biotypes.
```{r}
b2 %>% filter(isHgnc == "1") %>% group_by(gene_type) %>% count()
```
...
```{r}
b3 <- d %>% mutate(isHgnc = case_when(is.na(hgnc_id)~"0", TRUE ~ "1")) %>% filter(isHgnc=="1") %>% group_by(level) %>% count()
b3
```
...
```{r}
d %>% filter(type=="transcript") %>%
  count(gene_id)%>%summarize(gene_id[which.max(n)])
```
2. Please calculate the quantiles (0%, 25%, 50%, 75%, 100%) of the gene length for protein coding genes and long noncoding genes.

```{r}
d%>%filter(gene_type %in% c("protein_coding", "lncRNA")) %>% group_by(gene_type) %>% count(width) %>% summarise (width = quantile(width,c(0,0.25,0.50,0.75,1)), .groups ="keep") %>% mutate(q=c(0,0.25,0.50,0.75,1)) 
```
...
```{r}
b1 %>% filter(type == "gene") %>% group_by(seqnames, c_type) %>% count() %>% group_by(c_type) %>% summarise(mean(n),median(n))
```
...
```{r}
d %>% filter(type=="gene") %>% filter(gene_type %in% c("protein_coding", "lncRNA")) %>% group_by(seqnames, gene_type) %>% count()
```
